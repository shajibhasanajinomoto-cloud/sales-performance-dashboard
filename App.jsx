...
import { ChevronRight, ChevronDown, TrendingUp, TrendingDown, Target, Percent, LayoutDashboard, LineChart as LineChartIcon, ArrowLeft, Wifi, WifiOff, Loader2, Download, Calendar, CheckCircle2, AlertTriangle, Gauge } from "lucide-react";
...
export default function Dashboard() {
  ...
  const handleBrand = (b) => {
    setBrand(b);
    if (SKU_LIST[b]) setSku(SKU_LIST[b][0]);
  };
  const monthSubLabel =
    ...
    <div className="min-h-screen" style={{ background: CREAM, fontFamily: "'Inter', sans-serif" }}>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {page === "dashboard" && (
                <button
                  onClick={() => {
                    if (brand === TOTAL_LABEL) handleBrand(BRANDS[0]);
                    setPage("sku");
                  }}
      ...
      <>
        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          ...
          <KpiCard label="Progress" value={`${achPct(totals).toFixed(1)}%`} icon={Percent} accent={achPct(totals) >= 100 ? GREEN : RED} />
          {isCurrentMonthView && workingDayPace && (
            <div
              className="flex-1 min-w-[190px] rounded-2xl p-4 bg-white flex flex-col justify-between"
              style={{ boxShadow: "0 1px 2px rgba(10,38,71,0.04), 0 8px 24px -14px rgba(10,38,71,0.18)" }}
            >
              <div className="flex items-center justify-between mb-2">
                <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Pace</span>
                <div
                  className="w-7 h-7 rounded-lg flex items-center justify-center"
                  style={{ background: onTrack ? "rgba(31,146,84,0.12)" : "rgba(200,29,37,0.1)" }}
                >
                  {onTrack ? <CheckCircle2 size={14} style={{ color: GREEN }} /> : <AlertTriangle size={14} style={{ color: RED }} />}
                </div>
              </div>
              <div className="text-lg font-extrabold tracking-tight" style={{ color: NAVY, fontFamily: "'Sora', sans-serif" }}>
                Day {workingDayPace.passed} / {workingDayPace.total}
              </div>
              <div className="text-xs mt-1 font-semibold" style={{ color: onTrack ? GREEN : RED }}>
                {onTrack ? "On Track" : "Off Track"} · Expected {expectedPct.toFixed(0)}%
              </div>
            </div>
          )}
        </div>
