...
import { ChevronRight, ChevronDown, TrendingUp, TrendingDown, Target, Percent, LayoutDashboard, LineChart as LineChartIcon, ArrowLeft, Wifi, WifiOff, Loader2 } from "lucide-react";
...
// vertically in the sheet, so blanks are forward-filled from the row above.
function buildLiveSectionsWide(rawRows) {
  if (!rawRows || rawRows.length < 3) return null;
  const row1 = rawRows[0];
  const row2 = rawRows[1];

  // Forward-fill merged brand-group header cells (columns 3+ only)
  const filledRow1 = [];
  let lastBrand = "";
  ...
  for (let r = 2; r < rawRows.length; r++) {
    ...
    sectionMap[section][unit] = sectionMap[section][unit] || {};
    const cells = {};
    BRANDS.forEach((b) => (cells[b] = { target: 0, achv: 0, lastMonth: 0 }));
    sectionMap[section][unit][areaRaw] = cells;
  ...
  const result = Object.keys(sectionMap).map((secName) => ({
    name: secName,
...
function computeWorkingDayTrend(skuRows, brand, sku, maxDay = 26) {
...
function computeSkuComparisonLive(skuRows, brand) {
  const results = (SKU_LIST[brand] || [])
    .map((s) => {
      const t = computeWorkingDayTrend(skuRows, brand, s);
      if (!t) return null;
  ...
  return (
    ...
    >
      ...
      <div className="mt-2 flex gap-4 text-center">
        ...
        <div>
          <div className="text-[10px] uppercase tracking-wide text-slate-400">Achv</div>
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchCsvRaw(TARGET_CSV_URL), fetchCsv(SKU_CSV_URL)])
      .then(([targetRows, skuRows]) => {
        ...
        if (sections) setLiveSections(sections);
  ...
  const activeSections = liveSections || SECTIONS;
  ...
  const trend = useMemo(() => {
    if (liveSkuRows) {
      const t = computeWorkingDayTrend(liveSkuRows, brand, sku);
      if (t) return t;
    }
    return workingDayTrend(sku.length * 7 + brand.length);
  }, [sku, brand, liveSkuRows]);
  ...
  const skuComparison = useMemo(() => {
    if (liveSkuRows) {
      const c = computeSkuComparisonLive(liveSkuRows, brand);
      if (c) return c;
    }
  ...
  return (
    <div className="min-h-screen" style={{ background: CREAM, fontFamily: "'Inter', sans-serif" }}>
      ...
      <>
        ...
        {/* KPI row */}
        <div className="flex gap-3 mb-6 flex-wrap">
          <KpiCard label="Target" value={fmt(totals.target)} sub="KG, current month" icon={Target} accent={NAVY} />
          <KpiCard label="Achievement" value={fmt(totals.achv)} sub="KG, current month" icon={Percent} accent={GREEN} />
          <KpiCard label="Ach %" value={`${achPct(totals).toFixed(1)}%`} icon={Target} accent={achPct(totals) >= 100 ? GREEN : RED} />
        </div>

        {/* Hierarchy table */}
        <div className="bg-white rounded-2xl overflow-hidden mb-6" style={{ boxShadow: "0 1px 2px rgba(10,38,71,0.04), 0 8px 24px -14px rgba(10,38,71,0.18)" }}>
          <table className="w-full">
            <thead>
              <tr className="text-[11px] text-slate-400 uppercase tracking-wider border-b border-slate-100">
                ...
                <th className="text-right py-3 pr-4 font-semibold">Target</th>
                <th className="text-right py-3 pr-4 font-semibold">Achievement</th>
                <th className="text-right py-3 pr-6 font-semibold">Ach %</th>
              </tr>
            </thead>
            <tbody>
      ...
      <>
        {/* Brand selector for SKU page */}
        <div className="flex gap-1 bg-white rounded-xl p-1 mb-5 w-fit" style={{ boxShadow: "0 1px 2px rgba(10,38,71,0.04), 0 8px 24px -14px rgba(10,38,71,0.18)" }}>
          {BRANDS.map((b) => (
            <button
              key={b}
              onClick={() => handleBrand(b)}
