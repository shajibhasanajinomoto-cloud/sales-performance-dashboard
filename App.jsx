...
function computeGroupPerformance(skuRows, groupField, refMonth) {
...
function GroupLeaderboard({ title, data }) {
  ...
  return (
    <div className="bg-white rounded-2xl p-5" style={{ boxShadow: "0 1px 2px rgba(10,38,71,0.04), 0 8px 24px -14px rgba(10,38,71,0.18)" }}>
      ...
      <p className="text-xs text-slate-400 mb-4">Ranked by MoM growth — KG achieved this month, all brands combined.</p>
      <div className="divide-y divide-slate-100">
        {data.map((g, i) => {
          const isTop = data.length > 1 && g === top;
          const isBottom = data.length > 1 && g === bottom;
          return (
            <div key={g.name} className="flex items-center justify-between py-2.5 text-sm">
              <div className="flex items-center gap-2 min-w-0">
                <span
                  className="w-5 h-5 flex items-center justify-center rounded-full text-[10px] font-bold shrink-0"
                  style={{ background: isTop ? "rgba(31,146,84,0.15)" : isBottom ? "rgba(200,29,37,0.12)" : "#F1F5F9", color: isTop ? GREEN : isBottom ? RED : "#64748B" }}
                >
                  {i + 1}
                </span>
                <span className="font-medium text-slate-700 truncate">{g.name}</span>
                {isTop && <span className="text-xs shrink-0">🏆</span>}
                {isBottom && <span className="text-xs shrink-0">⚠️</span>}
              </div>
              <div className="flex items-center gap-3 shrink-0">
                <span className="text-xs text-slate-500 font-medium">{fmt(g.kg)} kg</span>
...
export default function Dashboard() {
  ...
  const districtPerf = useMemo(() => (liveSkuRows ? computeGroupPerformance(liveSkuRows, "district", selectedMonth) : []), [liveSkuRows, selectedMonth]);
  const teamPerf = useMemo(() => (liveSkuRows ? computeGroupPerformance(liveSkuRows, "team", selectedMonth) : []), [liveSkuRows, selectedMonth]);
  ...
  return (
    <div className="min-h-screen" style={{ background: CREAM, fontFamily: "'Inter', sans-serif" }}>
      ...
      <>
        ...
        {/* District & Team performance — quick view of who's ahead and who's lagging */}
        {(districtPerf.length > 0 || teamPerf.length > 0) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
            <GroupLeaderboard title="District Performance" data={districtPerf} />
            <GroupLeaderboard title="Team Performance" data={teamPerf} />
          </div>
        )}
